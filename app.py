...
labels = [
    ...
    "🌍 Macro", "🧬 Themes", "🤖 Ask Claude", "📤 Exports",
]
tabs = st.tabs(labels)
...
with tab_action:
    ...
    st.divider()
    if st.button("📋 Build a Claude briefing prompt", use_container_width=True):
        st.session_state["ac_prompt"] = ai_analyst.portfolio_briefing_prompt(
            summary, allocation, broker_total=broker_total,
            ugl_total=ugl_total, hhi=_hhi,
        )
    if st.session_state.get("ac_prompt"):
        st.caption("Copy this and paste it into Claude for a free narrative briefing:")
        st.code(st.session_state["ac_prompt"], language="markdown")
...
with tab_reb:
    ...
    if plan is not None:
        ...
        if plan.trades.empty:
            ...
            if cai.button("📋 Prompt to have Claude explain it", use_container_width=True):
                st.session_state["reb_prompt"] = ai_analyst.rebalance_prompt(
                    plan.trades, plan.stats)
            if st.session_state.get("reb_prompt"):
                st.code(st.session_state["reb_prompt"], language="markdown")
...
# -----------------------------------------------------------------------------
# Tab: Ask Claude  (prompt builder — no API key needed)
# -----------------------------------------------------------------------------
with tab_ai:
    section_header(
        "Ask Claude",
        "The app packs your live data into a ready-to-paste brief — copy it into "
        "Claude (or any assistant) for a free narrative. No API key required.",
    )
    st.caption(
        "Each button builds a self-contained prompt below. Hit the copy icon on the "
        "code block, paste it into a Claude chat, and you'll get the write-up back."
    )

    c1, c2 = st.columns(2)
    if c1.button("📋 Portfolio briefing", use_container_width=True):
        st.session_state["ai_prompt"] = ai_analyst.portfolio_briefing_prompt(
            summary, allocation, broker_total=broker_total,
            ugl_total=ugl_total, hhi=_hhi,
        )
    if c2.button("🔬 Explain my factor exposures", use_container_width=True):
        ...
        if ft is None:
            ...
            st.session_state["ai_prompt"] = ai_analyst.factor_prompt(
                ft, alpha=st.session_state.get("factor_alpha"),
                r2=st.session_state.get("factor_r2"),
                n_obs=st.session_state.get("factor_nobs"),
            )
    ...
    if pick and st.button(f"Build {pick} thesis prompt", use_container_width=True):
        row = summary[summary["Symbol"].eq(pick)].iloc[0].to_dict()
        st.session_state["ai_prompt"] = ai_analyst.thesis_prompt(pick, row)

    if st.session_state.get("ai_prompt"):
        st.divider()
        st.markdown("##### 📋 Copy this into Claude")
