...
def _technical_one(sym: str, period: str, interval: str) -> dict:
    ...
    try:
        ...
        if interval == "4h":
            # Intraday yfinance data is tz-aware; resample on a tz-naive copy
            # to keep 4h buckets aligned and avoid DST edge warnings.
            res_src = hist.copy()
            try:
                if res_src.index.tz is not None:
                    res_src.index = res_src.index.tz_localize(None)
            except (TypeError, AttributeError):
                pass
            hist = res_src.resample("4h").agg(
...
def fetch_live_change(symbols: Iterable[str], *, max_workers: int = 8) -> pd.DataFrame:
    """Live last price + today's % change via fast_info.

    Fallback for when the broker export's "Today's Change" column is empty
    (e.g. pre-market pulls). Uses fast_info.last_price vs previous_close so it
    is cheap (no full .get_info() call).

    Returns columns: Symbol, Live Last Price, Live Today's Change %.
    """
    syms = _dedupe_upper(symbols)
    if not syms or yf is None:
        return pd.DataFrame()
    ...
    def _one(sym: str) -> dict | None:
        try:
            fast = dict(yf.Ticker(yf_symbol(sym)).fast_info or {})
            ...
            return {
                "Symbol": sym,
                "Live Last Price": float(last),
                "Live Today's Change %": float(last) / float(prev) - 1.0,
            }
        except Exception:
            return None
