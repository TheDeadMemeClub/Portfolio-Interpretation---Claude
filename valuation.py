...
def fetch_advanced_valuations(
    ...
    *,
    ...
    Always returns a DataFrame containing every expected column (filled with
    NaN where data was unavailable) so downstream code can rely on the schema.
    """
    syms = list(dict.fromkeys(str(s).upper().strip() for s in symbols if str(s).strip()))
    if not syms or yf is None:
        return _ensure_valuation_schema(pd.DataFrame({"Symbol": syms}))
    ...
    return _ensure_valuation_schema(df)


# Columns every valuation dataframe is guaranteed to expose.
VALUATION_COLUMNS: tuple[str, ...] = (
    "Symbol", "Sector", "Industry", "Price", "Market Cap", "EPS (TTM)", "BVPS",
    "Trailing P/E", "Forward P/E", "Price/Sales", "Price/Book", "EV/EBITDA",
    "PEG (Yahoo)", "PEG (calc)", "PEG Verdict", "FCF (TTM)", "Op CF (TTM)",
    "Revenue Growth", "Earnings Growth", "ROE", "ROIC (proxy)", "Gross Margin",
    "Operating Margin", "Debt/Equity", "Dividend Yield", "Payout Ratio", "Beta",
    "Analyst Target", "Analyst High", "Analyst Low", "Analyst Upside %",
    "Recommendation", "DCF Fair Value", "DCF Upside %",
    "Implied Growth (Reverse-DCF)", "Graham Number", "Graham Upside %",
    "Owner Earnings Yield", "Valuation Verdict",
)


def _ensure_valuation_schema(df: pd.DataFrame) -> pd.DataFrame:
    """Add any missing expected columns (as NaN / blank) so callers are safe."""
    if df is None:
        df = pd.DataFrame()
    df = df.copy()
    for col in VALUATION_COLUMNS:
        if col not in df.columns:
            # Text-ish columns get empty string, numeric get NaN
            if col in ("Symbol", "Sector", "Industry", "Recommendation",
                       "Valuation Verdict", "PEG Verdict"):
                df[col] = "" if col != "Valuation Verdict" else "—"
            else:
                df[col] = np.nan
    return df
...
def quality_screen(valuation_df: pd.DataFrame) -> pd.DataFrame:
    ...
    (0-100) and Quality Tier column appended. Always adds both columns even if
    the input is empty or missing fundamentals.
    """
    df = valuation_df.copy() if valuation_df is not None else pd.DataFrame()
    if df.empty:
        df["Quality Score"] = pd.Series(dtype=float)
        df["Quality Tier"] = pd.Series(dtype=object)
        return df
