...
# ---------------------------------------------------------------------------
# fpdf2 with built-in fonts (Helvetica, etc.) only supports Latin-1.
# Any Unicode characters outside that range raise FPDFUnicodeEncodingException.
# We rewrite common typographic characters to ASCII equivalents and drop
# anything else that doesn't survive a Latin-1 round-trip.
_UNICODE_FALLBACKS = {
    "\u2014": "-",    # em dash
    "\u2013": "-",    # en dash
    "\u2022": "-",    # bullet
...
def _safe(text) -> str:
    """Make any string safe for the default fpdf2 Latin-1 fonts."""
    if text is None:
        return ""
    s = str(text)
    for k, v in _UNICODE_FALLBACKS.items():
        if k in s:
            s = s.replace(k, v)
    # Drop anything still outside Latin-1 (e.g. emoji in yfinance descriptions)
...
class _PortfolioPDF(FPDF if _HAS_FPDF else object):  # type: ignore[misc]
    def header(self):  # type: ignore[override]
        ...
        self.cell(0, 8, _safe("Portfolio EPIC - dashboard summary"),
                 new_x="LMARGIN", new_y="NEXT")
        self.set_font("Helvetica", "", 9)
        self.set_text_color(110, 110, 120)
        self.cell(0, 5, _safe(f"Generated {datetime.now().strftime('%Y-%m-%d %H:%M')}"),
                 new_x="LMARGIN", new_y="NEXT")
        self.ln(2)
...
def _add_table(pdf: "_PortfolioPDF", df: pd.DataFrame, col_widths: list[float]) -> None:
    ...
    for col, w in zip(df.columns, col_widths):
        pdf.cell(w, 5, _safe(str(col)[:22]), border=0, fill=True)
    pdf.ln()
